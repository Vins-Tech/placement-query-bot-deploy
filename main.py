...
def init_vector_store():
    ...
    # Silence any prints inside process_folder
    with contextlib.redirect_stdout(None):
        _ = list(process_folder(folder, reset=False))
    return True

# Initialize silently with controlled spinner
...
def ask(query):
    ...
    new_count = count + 1
    st.session_state["query_count"] = new_count  # immediate UI update

    # Async updates to avoid UI lag
    threading.Thread(target=update_query_count, args=(new_count,)).start()
    ...
    # async logging
    threading.Thread(target=log_entry, kwargs=dict(
        query=query,
...
- Using free APIs, therefore daily limit set to {MAX_QUERIES} (shared)
""")
st.sidebar.markdown("---")
st.sidebar.markdown("""
👨‍💻 **Contact:** Vinay S  
